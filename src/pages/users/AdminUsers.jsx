...
// Helper component để hiển thị Badge cho trạng thái
...
function EditUserModal({ user, onClose, onSaved }) {
  ...
  if (!user) return null
  return (
    <Modal isOpen={!!user} onClose={onClose} title="Chỉnh sửa Quản trị viên" size="small" subtitle={user.email}>
      <div className="modal-form user-edit-form">
        <div className="modal-form-section-card">
          ...
          <div className="modal-form-group" style={{ marginBottom: 16 }}>
            <label className="form-label">Quản trị viên</label>
          ...
          <div className="user-detail-hero-body">
            <div className="user-detail-hero-topline">
              ...
              <div className="user-detail-hero-tags">
                <BadgePill label="Quản trị viên" tone="warning" />
...
// ----------------------------------------------------------------------------------
// MAIN COMPONENT - ADMIN USERS ONLY
// ----------------------------------------------------------------------------------
export default function AdminUsers() {
  ...
  const load = async () => {
    try {
      ...
      // Lọc để chỉ lấy tài khoản back-office
      const adminUsers = data.filter(u => u.role && u.role !== 'user')
      setUsers(adminUsers)
    } catch (err) {
      console.error(err)
    }
  ...
  const remove = async (id) => {
    if (!confirm("Bạn có chắc chắn muốn xóa quản trị viên này?")) return
  ...
  return (
    <div className="admin-page">
      <PageHeader
        ...
        subtitle={`Tìm thấy ${filtered.length} trên tổng số ${users.length} quản trị viên`}
        icon={<FaCrown size={26} style={{ color: '#ef4444' }} />}
        actions={(
      ...
      <div className="table-section users-table-wrap bg-white p-4 rounded-lg shadow-md overflow-x-auto">
        {filtered.length === 0 ? (
          <EmptyState icon="👮" message="Không tìm thấy quản trị viên nào khớp với tiêu chí tìm kiếm và bộ lọc." />
        ) : (
